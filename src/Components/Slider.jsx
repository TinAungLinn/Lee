...
const Slider = () => {
  ...
  return (
    <div className=" overflow-x-hidden relative">
      ...
      >
        {data.map((img) => {
          return <Slide img={img} key={img.id} />;
        })}
      </div>
      <div className=" absolute z-[1] bottom-[50%] w-screen text-gray-50">
        <div className=" flex justify-between pr-5 pl-1 gap-10">
          <button onClick={prevSlide} className='  bg-black bg-opacity-50 text-sky-50 px-4 py-5'>
            <span>
              <BsArrowLeftShort className=" text-3xl"/>
            </span>
          </button>
          <button onClick={nextSlide} className='  bg-black bg-opacity-50 text-sky-50 px-4 py-5'>
            <span>
